...
One rule sits outside all of this: a `ref` puts `to` or `href` first, ahead of `rel`, which is not the order its schema file lists. [§6](#6-refs-and-combos) explains why. It's the only place this guide overrides the schema.
...
Guidelines sorts content by: **how broad first, who it's for second.** `for: agent`, `framing: when-to-use` still comes before a `for: all` `framing :how-to-use`, because breadth is settled first. Audience only sorts within the same `framing`.

Guidelines should lead with when to use, then describe how to use it. [`sections/guidelines.schema.yaml`](schema/sections/guidelines.schema.yaml) lists its `framing` values in:

1. **`framing: when-to-use`:** Does it make sense to use?
2. **`framing: how-to-use`:** If so, how do you use it?
3. **Tagged guidelines:** A tagged guideline section implies specific guidance. Narrow instruction should follow general information.

Sections with the same `framing` are ordered by how broad the audience is. The order [`sections/section.schema.yaml`](schema/sections/section.schema.yaml) lists its `for` values in:
...
Order items at the same level in whatever way aids comprehension. Ideally, group subject matter of the same level togther. Don't alphabetize items that share a level—that's dumb.
...
A `combo` follows [`common/combo.schema.yaml`](schema/common/combo.schema.yaml).
...
| `DSDS-19` | Guideline item order by `level` | [§5](#5-items-inside-one-section) |
...
**One thing isn't checked.** `DSDS-18` skips the "sections about one tag" part of [§4](#4-sections). Working out that a section "is really just the accessibility rules" means reading its items, not looking at its shape — unlike `framing` and `for`, which are plain fields. It's still worth following; it just isn't checked. If it turns out to be checkable after all, the rule to extend is `section-order` in `lint-docs.js`.

**Field order has one source: the schema files.** The tables on the [Schema page](https://designsystemdocspec.org/schema) are generated from each schema file's own order, `DSDS-17` and `DSDS-20` read that same order when they run, and [§2](#2-entries) tells you to follow it. So you can read the authoring order off a property table, and the other way round.
