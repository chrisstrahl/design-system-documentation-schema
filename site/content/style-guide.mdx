...
The last row is the exception, and a permanent one: "whatever order reads best" has no right answer a tool could compute, so nothing will ever check it. Order items sharing a `level`, or entries sharing a kind, the way that reads best and don't expect a warning either way.

The rest were unchecked until recently, and the gap was not theoretical. Nine violations had built up across this repo's own examples before anyone looked: seven combos with `level` and `items` the wrong way round, one `ref` leading with `rel`, and one section with `freeform` before `items`. Every rule that existed at the time was passing the whole while. That's what `DSDS-21`, `DSDS-22` and `DSDS-23` now cover.
